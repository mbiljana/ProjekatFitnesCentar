$(document).ready(function(){
    ...
    $.ajax({
        ...
        success: function (data) {                              // ova f-ja se izvršava posle uspešnog zahteva
            ...
            for (i = 0; i < data.length; i++) {
                ...
                row += "<td>" + data[i]['nazivCentra'] + "</td>";       // ubacujemo podatke jednog zaposlenog u polja
                row += "<td>" + data[i]['adresaCentra']  + "</td>";
                row += "<td>" + data[i]['brojTelefonaCentrale'] + "</td>";
                row += "<td>" + data[i]['emailCentra'] + "</td>";
    ...
    $("#obrisi").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "http://localhost:8181/api/sviFitnesCentri/brisanje",
            dataType: "json",
